...
double *ARmodel::PACF() {
    ...
    // cut-off after MAX_LAG lags
    int maxLags;
    if (data.sizeArray > MAX_LAG) {
        maxLags = MAX_LAG;
    } else {
...
void ARmodel::determineOrder() {
    ...
    for (int i = count; i < maxLags; i++) {
        if (*(significance + i)) {
            cout << "WARNING: Lag " + to_string(i) + " is significant and order is " + to_string(count) +
                    "! Maybe an AR model is not suitable." << endl;
        }
    }
